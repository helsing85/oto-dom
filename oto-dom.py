...
def readDataFromSite(url):
    ...
    if elems is not None:
        ...
        #Spacja oznacza że znacznik jest gdzieś niżej
        elems = soup.select("div.css-1l1r91c.emxfhao1 div.css-f45csg.estckra9 div.estckra8")
        ...
        with pandas.ExcelWriter(PLIK_DANE, mode="a", if_sheet_exists="overlay") as writer:
            for dom in dane:
                ...
                if strona_df is None:
                    logowanie('--- Brak oferty', plik_logow)
                else:
                    if nazwa_oferty in arkusze:
                        plik_df = xl.parse(nazwa_oferty)
                        polaczony_df = pandas.concat([strona_df, plik_df])
                        polaczony_df.to_excel(writer, sheet_name=nazwa_oferty, index=False) 
                    else:
                        strona_df.to_excel(writer, sheet_name=nazwa_oferty, index=False) 
    else:
        logowanie('Brak danych', plik_logow)
